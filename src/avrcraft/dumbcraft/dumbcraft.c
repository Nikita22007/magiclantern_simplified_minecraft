...
#include "dumbcraft.h"
#include <string.h>
#include "../libs/754lib.h"
#include "../libs/util10.h"
...
static uint8_t compeddata[] PROGMEM = {
...
static const char pingjson1[] PROGMEM = "{\"description\":\"";
static const char pingjson2[] PROGMEM = "\", \"players\":{\"max\":";
static const char pingjson3[] PROGMEM = ",\"online\":";
static const char pingjson4[] PROGMEM = "},\"version\":{\"name\":\""LONG_PROTO_VERSION"\",\"protocol\":"PROTO_VERSION_STR"}}";
...
{
	if( !cmdremain ) return;
//	uart_printf( "(+%d)", cmdremain );
	while( cmdremain-- )
	{
//		uart_printf( "%02x ", Rbyte() );
		Rbyte();
	}
...
//		uart_printf( "CMD: 0x%02x  REMAIN: %d COMP: %d\n", cmd, cmdremain, p->set_compression );
		...
		if( p->handshake_state == 0 )
		{
			if( cmd == 0 )
			{
				...
				{
#ifdef DEBUG_DUMBCRAFT
					uart_printf("WARNING: wrong version; got: %d; expected %d\n", i16, PROTO_VERSION );
#endif
...
#ifdef DEBUG_DUMBCRAFT
				uart_printf("Client switched mode to: %d\n", p->handshake_state );
#endif
			}
		}
		else if( p->handshake_state == 1 ) //Status
		{
			switch( cmd )
			{
				...
				default:
#ifdef DEBUG_DUMBCRAFT
					uart_printf( "Unknown status request (%d)\n", cmd );
#endif
					break;
			}
		}
		else if( p->handshake_state == 2 )
		{
			...
			case 0x00:
				...
				p->handshake_state = 3;
#ifdef DEBUG_DUMBCRAFT
				uart_printf( "Player \"%s\" Login, switching handshake_state.\n", p->playername );
				bmp_printf_auto("%s has joined the game!\n", p->playername);
			...
			default:
#ifdef DEBUG_DUMBCRAFT
				uart_printf( "Confusing packet for mode 2.\n" );
#endif
				break;
			}
		}
		...
		default:
#ifdef DEBUG_DUMBCRAFT
			uart_printf( "UCMD: (LAST) %02x - NOW: %02x\n", lastcmd, cmd );
			uart_printf( "UPKT:\n" );
			DumpRemain();
#endif
			break;
